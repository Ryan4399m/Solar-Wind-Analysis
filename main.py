...
cols = [
    "year", "day", "hour", "pts_lanl", "pts_mit", "pts_isee3",
    "speed_lanl", "speed_mit", "speed_isee3",
    "azimuth_lanl", "azimuth_mit", "azimuth_isee3",
    "density_lanl", "density_mit", "density_isee3",
    "temp_lanl", "temp_mit", "temp_isee3",
    "sigma_v_lanl", "sigma_v_mit", "sigma_v_isee3",
    "sigma_n_lanl", "sigma_n_mit", "sigma_n_isee3",
    "sigma_t_lanl", "sigma_t_mit", "sigma_t_isee3",
]

#functions to test
def replace_missing_values(df):
    return df.replace([999.9, 9999.9, 9999999., 99999.9, 999, 9999, 9999.], np.nan)

def filter_valid_days(df):
    return df[df["day"] > 0].copy()
...
def extract_isee(df):
    ...
    return isee.dropna().sort_index()

def compute_rolling_averages(isee):
    isee = isee.copy()
    isee["speed_24h"] = isee["speed_isee3"].rolling("24h").mean()
    isee["density_24h"] = isee["density_isee3"].rolling("24h").mean()
    return isee

def compute_correlation(isee):
    return isee["speed_isee3"].corr(isee["density_isee3"])
...
if __name__ == "__main__":
    ...
    isee = extract_isee(df)
    isee = compute_rolling_averages(isee)
